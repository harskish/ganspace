...
$(".save_button").on("click", function(){
    $(".save_modal_background").css('display', 'flex');
    component_number = this.getAttribute('id').split("_")[1];
    $(".save_modal_component_name").text("Comp: " + component_number + " Start: " + layer_start_value + " End: " + layer_end_value);

    $(".save_modal_cancel_button").on("click", function(){
        $(".save_modal_background").css('display', 'none');
    });

});

$(".save_modal_save_button").on("click", function(){
    var component_name = $(".save_modal_input_text").val();
    $.ajax({
        method: 'POST',
        // dataType: 'json',
        data: { 
